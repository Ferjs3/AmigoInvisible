...
public class RoomService {
    ...
    // El admin puede corregir estos datos en cualquier momento (no solo
    // mientras la sala esta abierta) -- por ejemplo, corregir el lugar
    // despues del sorteo si cambio el plan.
    @Transactional
    public RoomDetailResponse updateRoom(User admin, Long roomId, UpdateRoomRequest request) {
    ...
    public ExclusionResponse addExclusion(User admin, Long roomId, ExclusionRequest request) {
        ...
        return new ExclusionResponse(exclusion.getId(), giver.getId(), giver.getDisplayNameOrUsername(),
                receiver.getId(), receiver.getDisplayNameOrUsername());
    }
    ...
    public List<ExclusionResponse> listExclusions(User user, Long roomId) {
        ...
        return exclusionRepository.findByRoom(room).stream()
                .map(e -> new ExclusionResponse(e.getId(), e.getGiver().getId(), e.getGiver().getDisplayNameOrUsername(),
                        e.getReceiver().getId(), e.getReceiver().getDisplayNameOrUsername()))
                .toList();
    }
    ...
    public MyAssignmentResponse getMyAssignment(User user, Long roomId) {
        ...
        return new MyAssignmentResponse(assignment.getReceiver().getId(), assignment.getReceiver().getDisplayNameOrUsername());
    }
