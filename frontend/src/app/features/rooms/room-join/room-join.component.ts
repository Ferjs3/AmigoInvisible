...
export class RoomJoinComponent implements OnInit {
  ...
  ngOnInit(): void {
    // Si llegaste con un link de invitacion (ej. /rooms/join?code=XXXX,
    // compartido por WhatsApp), el codigo ya viene cargado y buscamos solo.
    const codeFromUrl = this.route.snapshot.queryParamMap.get('code');
    if (codeFromUrl) {
      this.code = codeFromUrl.toUpperCase();
      this.search();
    }
  }
